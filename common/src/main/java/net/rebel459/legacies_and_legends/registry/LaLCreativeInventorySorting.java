...
public class LaLCreativeInventorySorting {
	...
	private static void addAfterInNaturalBlocks(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.NATURAL_BLOCKS, comparedItem, item);
	}

	private static void addAfterInBuildingBlocks(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.BUILDING_BLOCKS, comparedItem, item);
	}

	private static void addAfterInFunctionalBlocks(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.FUNCTIONAL_BLOCKS, comparedItem, item);
	}

	private static void addAfterInToolsAndUtilities(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.TOOLS_AND_UTILITIES, comparedItem, item);
	}

    private static void addBeforeInToolsAndUtilities(ItemLike comparedItem, ItemLike item) {
        UnifiedHelpers.CREATIVE_ENTRIES.insertBefore(CreativeModeTabs.TOOLS_AND_UTILITIES, comparedItem, item);
    }

	private static void addBeforeInIngredients(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertBefore(CreativeModeTabs.INGREDIENTS, comparedItem, item);
	}

	private static void addAfterInIngredients(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.INGREDIENTS, comparedItem, item);
	}

	private static void addAfterInFoodAndDrinks(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.FOOD_AND_DRINKS, comparedItem, item);
	}

	private static void addAfterInCombat(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertAfter(CreativeModeTabs.COMBAT, comparedItem, item);
	}

	private static void addBeforeInCombat(ItemLike comparedItem, ItemLike item) {
		UnifiedHelpers.CREATIVE_ENTRIES.insertBefore(CreativeModeTabs.COMBAT, comparedItem, item);
	}
}
