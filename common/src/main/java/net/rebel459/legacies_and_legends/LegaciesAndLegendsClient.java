package net.rebel459.legacies_and_legends;

import net.minecraft.client.data.models.ItemModelGenerators;
import net.rebel459.legacies_and_legends.client.LaLEntityRenderers;
import net.rebel459.legacies_and_legends.client.LaLModelLayers;
import net.rebel459.legacies_and_legends.client.LaLStructureMusic;
import net.rebel459.legacies_and_legends.config.LaLConfig;
import net.rebel459.unified.platform.UnifiedHelpers;
import net.rebel459.unified.util.PackInfo;

public final class LegaciesAndLegendsClient {

    public void init() {
        LaLEntityRenderers.init();
        LaLModelLayers.init();
        LaLStructureMusic.init();

        TrimPaths.addPath("travelling_strides", ItemModelGenerators.TRIM_PREFIX_LEGGINGS);

        if (LaLConfig.get.music.music_and_melody) {
            UnifiedHelpers.PACKS.add(LaLConstants.id("music_and_melody"), PackInfo.REQUIRED_RESOURCES);
        }
        if (!LaLConfig.get.music.end_portal_music) {
            UnifiedHelpers.PACKS.add(LaLConstants.id("no_end_portal_music"), PackInfo.REQUIRED_RESOURCES);
        }
        if (UnifiedHelpers.PLATFORM.isModLoaded("farmersdelight") && LaLConfig.get.integrations.farmers_delight) {
            UnifiedHelpers.PACKS.add(LaLConstants.id("farmers_delight_integration"), PackInfo.REQUIRED_RESOURCES);
        }
    }
}
